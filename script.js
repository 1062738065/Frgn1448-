...
/* =============================== Supabase integration (optional) =============
   Paste your Supabase project URL and anon key below (see the accompanying
   supabase-setup.sql file for setup steps). When these are empty, the app
...
async function supabaseRequest(path, options) {
  options = options || {};
  if (!sheetsConfigured()) return { ok: false, error: "لم يتم ربط الموقع بقاعدة البيانات بعد" };
  try {
    ...
    if (!res.ok) {
      let msg = "خطأ من قاعدة البيانات (" + res.status + ")";
      try { const j = await res.json(); msg = j.message || msg; } catch (e) { /* تجاهل */ }
...
function renderLogin() {
  ...
  <div class="login-wrap">
    <div class="login-box">
      ...
      ${sheetsConfigured()
        ? `<div class="login-hint" style="color:${GREEN}">مربوط بقاعدة البيانات ✓ — سجّلي بالاسم وكلمة المرور المسجّلة بها</div>`
...
async function refreshGoalsDefinitionsFromSheet() {
  ...
  if (res.ok && Array.isArray(res.data)) {
    S.goalsDefinitions = {
      strategic: res.data.filter((r) => r.kind === "strategic").map(rowToGoal),
